...
function changeTexto(newTexto, estilo) {
  const texto1 =
    "Los estudiantes que son muy diligentes a la hora de estudiar tiene buenas notas, pero los que no se esfuerzan no logran nunca porder llegar mas lejos.";
  const texto2 =
    "Students who are very diligent when it comes to studying get good grades, but those who don't make an effort never manage to get further.";
  const texto3 =
    "非常に勤勉に勉強する生徒は良い成績を収めますが、努力しない生徒は決してそれ以上の成績を収めることはできません。.";
...
function misparrafos() {
  const p0 =
    "La programacion web es una materia muy divertida que puede ser muy util, si se aplica los conocimientos dados por el profesor";
